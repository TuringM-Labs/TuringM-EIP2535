...
export const prepareUserOrder = async marketId => {
    ...
    const facet = await getFacetWithSignerKey('TuringMarketApp', 'MatchOrderSelfFacet', 'orderMatcher1')
    const users = await getUnnamedAccounts();

    for (let i = 0; i < 7; i++) {
        await permitERC20('USDTMock', users[i], 'TuringMarketApp', 'deployer')
        await permitERC20('TuringToken', users[i], 'TuringMarketApp', 'deployer')
